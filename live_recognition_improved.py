...
class LiveFaceRecognition:
    ...
    def start_recognition(self):
        ...
        # Keep track of recognized faces and timestamps
        recognized_faces = {}
                ...
                for i, face_loc in enumerate(face_locs):
                    ...
                    try:
                        ...
                        if True in matches:
                            ...
                            # Add to recognized faces dictionary
                        ...
                        cv2.rectangle(display_frame, (left, label_y-35), (right, label_y), (0, 255, 0), cv2.FILLED)
                        font = cv2.FONT_HERSHEY_DUPLEX
                        cv2.putText(display_frame, name, (left + 6, label_y-6), font, 0.6, (255, 255, 255), 1)
                        
                        # Print name in terminal and add more prominent display for recognized faces
                        if name != "Unknown":
                            ...
                            notification_y = 30
                            cv2.rectangle(display_frame, (10, notification_y-text_height-10), 
                                         (text_width+20, notification_y+10), (0, 255, 0), cv2.FILLED)
                            cv2.putText(display_frame, text, (15, notification_y), 
                                       font, font_scale, (255, 255, 255), 2)
                    except Exception as e:
                        print(f"Error matching face: {e}")
                        continue
                
                # Display names of all recognized people at the bottom of the frame
                if recognized_faces:
                    y_pos = display_frame.shape[0] - 10
                    cv2.putText(display_frame, "Recognized People:", (10, y_pos - 40), 
                               cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 255, 255), 1)
                    
                    for i, name in enumerate(recognized_faces.keys()):
                        cv2.putText(display_frame, f"- {name}", (20, y_pos - 10 + i*(-30)), 
